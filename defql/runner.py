...
from .base import TableNode, flatten_ctes
...
class Runner:
    ...
    def build_statements(self, ctx: Context, target: str) -> list[str]:
        ...
        for name in order:
            node = ctx.nodes[name]
            ctes = flatten_ctes(node.ctes)
            sql = self._inject_ctes(node.spec.sql, ctes)
    ...
    def _inject_ctes(self, sql: str, ctes: list[TableNode]) -> str:
        if not ctes:
            return sql
    ...
    def _build_cte_exprs(self, ctes: list[TableNode]) -> list:
        exprs = []
        for cte in ctes:
            cte_query = sqlglot.parse_one(cte.spec.sql, dialect="duckdb")
            exprs.append(
                sqlglot_exp.CTE(
                    this=cte_query,
                    alias=sqlglot_exp.TableAlias(
                        this=sqlglot_exp.to_identifier(cte.spec.name),
                    ),
                )
            )
        return exprs
